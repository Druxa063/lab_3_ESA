...
public class ApplicationController {
    ...
    private String getHttp(String suffix){
    ...
    private String postHttp(String suffix, Object object){
        RestTemplate template = new RestTemplate();
        HttpHeaders httpHeaders = new HttpHeaders();
    ...
    private String httpRequest(String suffix, HttpMethod method, Object object, HttpHeaders httpHeaders){
        RestTemplate template = new RestTemplate();
        HttpEntity httpEntity = new HttpEntity(object, httpHeaders);
        ResponseEntity<String> entity = template.exchange(URI.create(SERVER_URI + suffix), method, httpEntity, String.class);
        return entity.getBody();
    }

    private Document getDocument(String element) {
    ...
    public String watchGroupsByLesson(Model model, @RequestParam("id") String id){
        String element = getHttp("groupsByLesson");
        Document document = getDocument(element);
        model.addAttribute("groups", document);
        return "groups";
    }
    ...
    public ModelAndView deleteGroup(ModelMap model, @RequestParam("id") String id){
        HttpHeaders httpHeaders = new HttpHeaders();
        httpHeaders.add("Content-type", MediaType.APPLICATION_JSON_VALUE);
        httpRequest("deleteGroup?id=" + id, HttpMethod.DELETE, null, httpHeaders);
        return new ModelAndView("redirect:/groups", model);
    }
    ...
    private void addToModel(Model model, String path){
        String element = getHttp(path);
        Document document = getDocument(element);
        Element temp = document.createElement("teachers");
        temp.setNodeValue(getHttp("teachers"));
    ...
    public ModelAndView deleteLesson(ModelMap model, @RequestParam("id") String id){
        HttpHeaders httpHeaders = new HttpHeaders();
        httpHeaders.add("Content-type", MediaType.APPLICATION_JSON_VALUE);
        httpRequest("deleteLesson?id=" + id, HttpMethod.DELETE, null, httpHeaders);
        return new ModelAndView("redirect:/lessons", model);
    }
    ...
    public String watchTeachers(Model model){
        String element = getHttp("teachers");
        Document document = getDocument(element);
    ...
    private ModelAndView saveTeacher(Teacher teacher, String firstName, String lastName, String position, ModelMap model){
        ...
        postHttp("createTeacher", teacher);
    ...
    public ModelAndView deleteTeacher(ModelMap model, @RequestParam("id") String id){
        HttpHeaders httpHeaders = new HttpHeaders();
        httpHeaders.add("Content-type", MediaType.APPLICATION_JSON_VALUE);
        httpRequest("deleteTeacher?id=" + id, HttpMethod.DELETE, null, httpHeaders);
        return new ModelAndView("redirect:/teachers", model);
    }
